...
R1 = Link('d',0,'a',0.05,'alpha',0) 
R2 = Link('d',0.05,'a',0,'alpha',0) 

            
gripperRight = SerialLink([R1 R2],'name','rightRobot') 


qRight = [0,0];

%Left finger gripper
L1 = Link('d',0,'a',-0.05,'alpha',0) 
L2 = Link('d',0.05,'a',0,'alpha',0) 

gripperLeft = SerialLink([L1 L2],'name','leftRobot') 
...
%Plotting the two grippers

gripperRight.plot(qRight)
gripperLeft.plot(qLeft)
...
brickStart = [[-0.7,-0.3,0];[-0.5,-0.3,0];[-0.3,-0.5,0];
             [-0.7,-0.4,0];[-0.5,-0.4,0];[-0.3,-0.4,0];
             [-0.7,-0.5,0];[-0.5,-0.5,0];[-0.3,-0.3,0]];
...
brickEnd = [[-0.6,0.4,0];[-0.4666,0.4,0];[-0.3332,0.4,0];
           [-0.6,0.4,0.0334];[-0.4666,0.4,0.0334];[-0.3332,0.4,0.0334];
           [-0.6,0.4,0.0334*2];[-0.4666,0.4,0.0334*2];[-0.3332,0.4,0.0334*2]];
